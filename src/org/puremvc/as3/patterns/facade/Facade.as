...
package org.puremvc.as3.patterns.facade
{
	...
	public class Facade implements IFacade
	{
		...
		{
			var proxy:IProxy;
			if ( model != null ) proxy = model.removeProxy ( aProxyName );	
			return proxy
		}
		...
		{
			if ( view != null ) view.registerMediator( aMediator );
		}
		...
		public function removeMediator( aMediatorName:String ) : IMediator 
		{
			...
			if ( view != null ) mediator = view.removeMediator( aMediatorName );			
			return mediator;
		}
		 ...
		 */ 
		public function sendNotification( aNotificationName:String, aBody:Object=null, aType:String=null ):void 
		{
			notifyObservers( new Notification( aNotificationName, aBody, aType ) );
		}
		...
		public function notifyObservers ( aNotification:INotification ):void {
			if ( view != null ) view.notifyObservers( aNotification );
		}
		...
		// Message Constants
		protected static const SINGLETON_MSG	: String = "Facade Singleton already constructed!";
